...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(Object.keys(createMenu(testeError).fetchMenu())).toEqual(["food", "drink"]);
    ...
    // ```
    expect(createMenu(testeError).fetchMenu()).toEqual(testeError);
    ...
    // ```
    expect(createMenu(testeError).consumption).toEqual([]);
    ...
    // ```
    const coxinha = createMenu(testeError);
    ...
    // ```
    const multItem = createMenu(testeError);
    multItem.order("agua");
    multItem.order("sopa");
    multItem.order("cerveja");
    expect(multItem.consumption).toEqual(['agua', 'sopa', 'cerveja']);
    ...
    // ```
    const equalItem = createMenu(testeError)
    equalItem.order('coxinha');
    equalItem.order('agua');
    equalItem.order('coxinha');
    equalItem.order('agua');
    expect(equalItem.consumption).toEqual(['coxinha', 'agua', 'coxinha', 'agua']);
    ...
    // ```
    const bill = createMenu(testeError);
    bill.order('sopa');
    bill.order('agua');
    bill.order('coquinha');
    bill.order('coxinha');
    bill.order('cerveja');
    expect(bill.pay()).toEqual(35.86);
